...
- `corpora/corpus_v2_coverage_matrix.json`：基础事实、多来源、表格、扫描/OCR、图片/视觉、拒答、版本冲突、重复、多格式和网页快照覆盖矩阵；`planned_count` 是待收集蓝图，`current_count` 只统计已审核可执行资料或非 blocked 题，缺口按 `current_count` 计算。
...
- `plans/corpus_v2_no_cost_preflight_v1.json` 与 `reviews/corpus_v2_preflight_summary.md`：零真实供应商调用的预检摘要；当前 10 个 documents 均非 ready、8/9 题 blocked，覆盖缺口为 10 类，Embedding、LLM、Rerank、OCR、Vision 调用数均为 0。
