...
def assert_call_allowed(*, attempted_hashes: list[str], requested_hash: str, max_calls: int) -> None:
    if requested_hash in attempted_hashes:
        raise VisionPolicyError("相同图片观察目标不能重复执行")
    if len(attempted_hashes) >= max_calls:
        raise VisionPolicyError("该图片已达到最多 3 次观察上限，请上传更清晰的图片")
    focused_count = sum(item != OVERVIEW_HASH for item in attempted_hashes)
