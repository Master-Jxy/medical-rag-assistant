...
const editing = ref(null), quotaForm = ref({ plan_code: 'free', token_limit_override: '', request_limit_override: '', reason: '' })
...
function openQuota(user) {
  editing.value = user
  quotaForm.value = { plan_code: 'free', token_limit_override: user.token_limit || '', request_limit_override: '', reason: '' }
}
async function saveQuota() {
  try {
    await adjustUserQuota(editing.value.user_id, {
      plan_code: quotaForm.value.plan_code,
...
<template>
  <section class="platform-page">
    ...
    <template v-else-if="overview">
      <section class="metric-grid"><article><span>总请求</span><strong>{{ overview.requests }}</strong></article><article><span>总Token</span><strong>{{ overview.total_tokens.toLocaleString() }}</strong></article><article><span>估算费用</span><strong>¥{{ overview.estimated_cost_cny.toFixed(4) }}</strong></article><article><span>计量覆盖率</span><strong>{{ (overview.measurement_coverage * 100).toFixed(1) }}%</strong></article><article><span>平均耗时</span><strong>{{ overview.average_latency_ms == null ? '暂无' : `${overview.average_latency_ms} ms` }}</strong></article><article><span>首Token耗时</span><strong>{{ overview.average_time_to_first_token_ms == null ? '暂无' : `${overview.average_time_to_first_token_ms} ms` }}</strong></article><article><span>未知计量</span><strong>{{ overview.unknown_calls }}</strong></article><article><span>失败调用</span><strong>{{ overview.failed_calls }}</strong></article></section>
    ...
    <div v-if="editing" class="modal-backdrop" role="presentation"><form class="quota-dialog" @submit.prevent="saveQuota"><h2>调整 {{ editing.email }} 的额度</h2><label>计划<input v-model="quotaForm.plan_code" required maxlength="32"></label><label>Token上限<input v-model="quotaForm.token_limit_override" type="number" min="1"></label><label>请求上限<input v-model="quotaForm.request_limit_override" type="number" min="1"></label><label>调整原因<textarea v-model="quotaForm.reason" required minlength="3" maxlength="200"></textarea></label><p>修改会写入超级管理员审计，历史用量账本不会改变。</p><div><button type="button" @click="editing = null">取消</button><button type="submit">确认调整</button></div></form></div>
  </section>
</template>
