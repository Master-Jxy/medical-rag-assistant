...
defineEmits(['new', 'select', 'rename', 'archive', 'delete', 'legacy'])
</script>
...
<template>
  <aside class="thread-sidebar" aria-label="Agent会话列表">
    ...
    <div v-else-if="!threads.length" class="empty">暂无Agent会话。</div>
    <div v-for="thread in threads" :key="thread.id" class="thread-row">
      ...
      <div class="thread-actions">
        ...
        <button :aria-label="`归档${thread.title}`" @click="$emit('archive', thread)">归档</button>
