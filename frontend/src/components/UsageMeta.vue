...
const label = computed(() => {
  ...
  if (usage.measurement === 'unknown') return `模型实际 Token 未知 · 额度扣减 ${charged} Token`
  if (usage.measurement === 'not_applicable') return '未调用模型 · 实际 0 Token · 额度扣减 0 Token · ¥0'
  const cost = usage.estimated_cost_cny == null ? '单价未配置' : `估算 ¥${Number(usage.estimated_cost_cny).toFixed(4)}`
  return `模型实际 ${Number(usage.total_tokens || 0).toLocaleString()} Token（输入 ${Number(usage.input_tokens || 0).toLocaleString()} / 输出 ${Number(usage.output_tokens || 0).toLocaleString()}） · 额度扣减 ${charged} Token · ${cost}`
})
</script>
