...
- 当前生产运行`Platform v1.4`与`Cloud v2.1`部署基线；自动备份、隔离恢复演练、
  受信任公网IP HTTPS、自动续期和整机重启恢复已经通过。
- 当前没有自有品牌域名；正式域名必须由用户持有并满足适用的DNS/备案条件，不能把
  `sslip.io`或公网IP写成自有品牌域名。
...
5. 先重建后端并检查健康；前端有源码变化时必须在本地重新运行测试和正式构建。`frontend/dist`不进入Git，发布者需上传新的dist、逐文件比对SHA-256并确认生产HTML引用新资源名后再重建`web`；禁止仅在服务器执行`docker compose build web`而复用旧dist。不得删除或重建命名卷。
