...
本文件是长期架构档案，不是每个开发窗口的全文必读材料。先读 `AGENTS.md` 与 `docs/handoff.md`，再按handoff只读取本任务涉及的章节。当前状态和唯一下一任务以handoff为准，历史发布事实以Git与发布审计为准。
...
以下内容描述阶段五建立的认证与隔离基线。阶段九已经按第12节完成并发布“用户提交、
管理员审核、发布后公开检索”，因此下列文档权限属于历史基线，不代表当前上传流程。
...
- 普通用户只能删除自己上传的文档。
- 系统预置文档不能由普通用户删除。
- 阶段五的基础登录版本不做邮箱验证码、找回密码和Refresh Token；阶段16已经在此
  基线上发布邮箱验证、密码重置和`token_version`，但仍不引入Refresh Token。
...
阶段16已经在不改变上述登录、隔离和Bearer Token基线的前提下发布验证码发送、忘记
密码和旧JWT失效接口。接口、迁移和安全边界见第19节。
...
任务7.8第一小步的本地审计记录在`docs/release-audit-rag-v1.2.1.md`。审计基准为`main`分支提交`82d1cb94bc3a11bb06ceb833cfbda747d7d1dd52`，候选范围覆盖任务7.1～7.7的源码、测试、无正文评估资产和文档；真实`.env`、Chroma、上传文件、备份、本地回答、缓存、依赖和构建产物均通过Git忽略规则排除。用户确认后，第二小步按清单完成提交和GitHub推送；在独立服务器授权后，第三小步先生成并校验MySQL、Chroma、上传文件和Redis四类备份，再把线上仓库同步到指定提交`9c4cd22e84fed5b02ddffcb0c4f3e5bdc380f6a2`并只重建后端容器。线上工作区保持干净，真实`.env`和持久数据未被覆盖，生产仍为冻结向量基线且混合检索、Reranker关闭。健康、迁移、免费认证/会话/文档接口和后端容器重启恢复通过。

线上验收临时工具后来暴露两个独立问题：正式SSE token事件的数据字段是`content`，旧工具却读取`token`，因此会把非空事件错误拼成空答案；错误报告函数在`set +e`后返回原错误码但没有显式退出，使主流程继续执行。当前版本化校验器严格读取`content`，同时验证数量、四个腔室标准或常见压缩同义写法、上传来源、`done`和无`error`；旧字段会被明确拒绝。临时Shell工具的ERR trap会先移除自身、输出脱敏阶段/request_id、执行补偿清理并显式退出。固定SSE、反向样例和服务器Bash失败即停自测均无费用通过，随后经独立费用确认完成了最终真实验收。

用户按预检预算确认后，最终受控验收完成3次Embedding业务调用（1次临时上传、2次查询）和2次Qwen SSE，未调用Reranker且客户端重试固定为0。临时医学资料上传使MySQL/Chroma/文件从27/103/27同步变为28/104/28；正常流返回19个token事件和4个来源，四腔室数量/名称、临时上传来源及`done`全部通过。第二条长回答在首个token后请求停止，73毫秒收到`stopped`；两条助手消息分别持久化为`completed`和`stopped`，生成锁已释放，停止接口回到`idle`，复用第一条幂等键只回放既有结果而未增加模型调用。删除临时文档、会话和账号后恢复27/103/27、4账号、5会话和40消息，五类定向残留均为0，四容器健康。任务7.8及阶段七据此完成。
...
1. JSON 结构化日志和统一字段。
2. 请求、检索、模型、工具四类阶段耗时。
3. Token和费用字段与聚合；当前模型适配器没有可靠计量时明确返回`unknown`/`null`，不伪造数据。
4. 管理员只读统计接口或简单页面。
5. 限流、Redis 降级、失败和中止计数。

### 11.1 事件、Port与调用方向

`TelemetryEvent`是只含固定安全字段的不可变事件，`TelemetryPort`只负责提交事件，`TelemetryMetricsPort`额外提供聚合快照。HTTP中间件建立`request_id`上下文并写入响应头；认证成功后只登记内部用户ID。`RagService`分别记录查询构造、知识检索、可选重排和模型生成的单调时钟耗时，停止或异常同样结束阶段事件。
...
-> Rotating JSONL file + in-process metrics
```

业务服务不导入具体日志适配器；`emit_safely`以及适配器内部隔离保证日志或聚合失败不会改变认证、文档、检索、回答和停止结果。`TELEMETRY_ENABLED=false`时使用空实现，原业务继续运行，管理员统计返回空快照。

### 11.2 日志轮转、保留与敏感信息

本地默认日志为`backend/logs/telemetry.jsonl`；Docker默认写入已有`app_data`持久卷中的`/app/data/logs/telemetry.jsonl`。单文件上限5 MiB，最多保留5份备份，加当前文件理论上限约30 MiB。路径、单文件大小和备份数量分别由`TELEMETRY_LOG_PATH`、`TELEMETRY_LOG_MAX_BYTES`和`TELEMETRY_LOG_BACKUP_COUNT`控制；日志目录被Git忽略。轮转适配器只序列化事件类型定义的字段，HTTP只记录路径而不记录查询参数、Header或Body，因此完整问题、回答、Prompt、密码、JWT、API Key、邮箱和医学正文不能进入该日志。

### 11.3 聚合指标与管理员只读接口

进程内聚合记录请求总数/成功/失败/平均耗时、各阶段平均耗时、限流、Redis降级、用户停止、模型/检索/持久化失败和主要错误类型。它不依赖Redis，进程重启后从零开始，不作为审计账本。模型使用量只有在适配器提供可靠实际值或明确估算时才计入；否则Token标记`unknown`，费用返回`null`。

管理员通过`GET /api/v1/admin/telemetry/stats`读取聚合数据，统一`require_admin`授权；普通用户稳定返回403。Vue使用独立`adminTelemetry.js`和`AdminTelemetryView.vue`，路由为`/admin/telemetry`，不把统计逻辑写进聊天或知识库页面。接口和页面都只展示聚合值，不提供日志全文。
...
生产已经使用Certbot 5.7为`112.124.9.120`签发受信任短期证书；HTTP 308、HTTPS健康、
真实SSE、续期dry-run、web重启和整机重启均通过。自动续期timer是可用性的组成部分，
证书剩余有效期低于24小时应视为生产告警。自有品牌域名仍需用户提供所有权和适用备案，
属于运营配置，不改变模块或部署架构。
...
## 19. 阶段十六：邮箱账号生命周期与模型用量计量 `[已完成并发布]`
...
RAG / Agent
-> QuotaGatePort / UsageRecorderPort
-> usage应用服务
-> MySQL用量账本、额度周期与reservation
```

阶段17已消除两个原有耦合：摘要写入移动到消息完成钩子，普通RAG和Agent统一通过
`MemoryContextProvider`按当前问题返回少量相关active记忆。敏感健康记忆未经用户确认
不得生效，记忆内容不得作为系统指令。

厂商无关`ModelUsage`已迁入usage模块，RAG Port保留兼容重导出；回答级分组、个人查询、
管理员查询和额度均由usage模块拥有。
额度预留以MySQL事务和行锁为真相，Redis只允许作短期缓存；模型用量账本不得因额度调整
被重写。

本地已增加`0022_memory_v2`、`0023_usage_groups`、`0024_user_quota`及对应的memory/usage
Port、Repository、应用服务、个人中心和管理员入口。提取任务先提交running/attempt，
模型异常保存稳定错误码并只有限重试；新鲜running不会被并发恢复，过期任务由独立Session
恢复。记忆整理usage单独记录为`surface=memory`且不计入聊天额度。

RAG和Agent在模型链路前通过统一Quota Gate预留；首次周期先锁用户行，再锁/创建周期，
避免并发首次请求创建窗口。actual按真实usage结算、unknown按预留结算、
not_applicable扣0；过期reservation按回答账本settle或release。Agent客户端断开会先关闭
...
Stage 23 已完成无费用临时 SQLite 和相关后端回归测试，并以提交 684e4c1 发布。
发布前完整备份和 SHA-256 校验通过，只重建 backend；MySQL、Redis、Web 和命名卷
...
The coverage matrix is deterministic and separates planned coverage from
currently executable coverage. `planned_count` records the collection blueprint
including placeholder documents and blocked cases. `current_count` is the only
field used for gap calculation: document-driven categories count only documents
with `ingestion_status="ready"`, `governance.status="current"`, and a concrete
content SHA-256; case-driven categories count only cases whose
`expected_behavior` is not `blocked`. `blocked_count`, `planned_document_ids`,
`evidence_document_ids`, `executable_case_ids`, and `blocked_case_ids` keep the
reason visible without overstating readiness. With the initial Stage 24.7
placeholder manifest, all 10 document-driven coverage items have current count
0; only the refusal case is executable, so the no-cost preflight reports 10
coverage gaps. `eval_v2` uses a versioned schema with explicit
`expected_behavior`; provider-dependent OCR/Vision cases must remain `blocked`,
and unanswered fixture placeholders may not invent golden answers.
...
dataset references, coverage, deterministic report checksums, and a budget
gate. The no-cost gate is derived from two conditions: current provider calls
must all be zero and the preflight must not be in provider-execution mode. The
