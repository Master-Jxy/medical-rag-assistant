...
Redis 已承载注册、登录、四个聊天入口的限流，普通/管理员上传的频率和并发保护，以及带会话问答的生成锁与请求幂等。Agent 仍未实现，不能画进“当前组件”冒充现状。
...
- 普通文档上传、管理员新增系统文档和管理员整份替换共享同一份按账号额度；默认每个用户 3600 秒最多 5 次，同时最多处理 1 个上传。
...
部署放在 Redis 保护、RAG 质量基线、可观测性和 Agent MVP 稳定之后。若求职展示需要提前上线，可以从完成任务 6.3 后冻结一个 `RAG v1` 发布版本，但不能一边改数据库和 Agent 一边直接修改线上唯一实例。
