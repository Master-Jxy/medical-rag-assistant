...
- 普通上传、管理员新增和管理员替换共享按用户的小时级额度与带所有权/TTL 的并发占位，拒绝发生在文档生命周期和 Embedding 之前。
...
部署基线已于 2026-07-17 完成：仓库具备 MySQL、Redis、FastAPI、Vue/Nginx 的 Compose 编排、持久卷、健康检查和资源上限，并已在 Ubuntu 22.04、2 核 2G 阿里云 ECS 上通过公网首页、健康接口、注册、登录、当前用户、创建会话和文档列表验收。当前仅开放 HTTP，真实账号不得复用其他网站密码；部署和更新命令以 `docs/deployment.md` 为准。
...
- 后端：112 项通过；新增“模型下一块阻塞 30 秒时仍在 0.5 秒上限内取消”的回归测试，并覆盖 `stopped` 持久化、幂等清理和停止后锁释放；原认证、限流、会话、上传、文档补偿和管理员测试无回归。
- Vue：7 个测试文件、26 项测试通过，覆盖原登录闭环、文档权限、管理员交互、稳定错误提示、SSE 读取器取消以及等待后端停止确认后恢复发送。
- SSE UTF-8 字节分片测试：通过。
- Vite 正式构建：通过。
