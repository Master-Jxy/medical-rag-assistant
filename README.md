...
一个面向医学资料学习与内部知识检索场景的前后端分离 RAG 应用。系统支持 PDF/TXT/DOCX/Markdown/HTML 文档管理和受控网页快照导入、向量检索、流式问答、引用来源、多轮会话和刷新恢复。
...
- FastAPI + Vue 3 前后端分离。
- PDF/TXT/DOCX/Markdown/HTML 上传、受控网页快照导入、解析、递归切分和 SHA-256 内容去重。
...
- `Agent Chat v3.1`已经完成并上线：代码、迁移、两栏聊天界面、确定消息顺序、旧入口
  与旧记录退役、直接回答/澄清/按需工具路由、在线无费用与真实模型验收均已通过。
...
Agent连续任务 -> Agent会话/消息 -> 关联多次运行 -> 最近上下文/显式产物引用 -> 新产物
```
...
- **公共知识库**：提交 PDF/TXT/DOCX/Markdown/HTML 或网页快照、查看已发布资料；提交成功不等于已发布。
- **管理中台**：审核、知识资产、任务、审计、系统资料与运行统计。
- **超级管理**：用户角色和账号状态治理。
- **Agent Chat**：管理活动/归档会话，连续运行受控资料任务，查看公开计划、工具步骤、
  来源、显式引用和可下载产物。
...
-> 阶段19 额度策略v2（已发布，shadow观察中，enforce未启用）
